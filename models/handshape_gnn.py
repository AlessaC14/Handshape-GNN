#!/usr/bin/env python3
"""
HandshapeGNN: Static Graph Neural Network for Handshape Recognition

This model processes individual static frames with low motion to learn
handshape-specific representations using anatomically-informed graph structures.
...
class HandshapeGNN(torch.nn.Module):
    """
    Graph Neural Network for processing static handshape configurations.

    Architecture:
        - 3 GCN layers with LeakyReLU activation
        - Batch normalization after each layer
        - Dropout for regularization
        - Global mean pooling for hand-level embeddings

    The graph structure uses anatomically-informed bidirectional edges:
        - Sequential: finger joint connections
        - Cross-finger: connections between adjacent fingers
        - Palm-centered: connections from wrist to finger bases
        - Diagonal palm: connections across the palm
    ...
    def __init__(self, input_dim=3, hidden_dim=64, embedding_dim=32):
        super(HandshapeGNN, self).__init__()
    ...
    def forward(self, x, edge_index, batch):
        ...
        """
        # First GNN layer - encode individual keypoint positions
        x = self.conv1(x, edge_index)
        x = self.bn1(x)
        x = F.leaky_relu(x)
        x = F.dropout(x, p=0.2, training=self.training)

        # Second GNN layer - capture finger-level patterns
        x = self.conv2(x, edge_index)
        x = self.bn2(x)
        x = F.leaky_relu(x)
        x = F.dropout(x, p=0.2, training=self.training)

        # Third GNN layer - learn complete hand configuration
        x = self.conv3(x, edge_index)
        x = self.bn3(x)
        x = F.leaky_relu(x)

        # Global pooling to get hand-level embedding
